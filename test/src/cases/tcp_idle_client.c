...
{
    ASSERT_EQ_D32(ev_loop_init(&g_test_43e2.s_loop), 0);
    ASSERT_EQ_D32(ev_tcp_init(&g_test_43e2.s_loop, &g_test_43e2.l_sock), 0);
    ASSERT_EQ_D32(ev_tcp_init(&g_test_43e2.s_loop, &g_test_43e2.s_sock), 0);
    ASSERT_EQ_D32(ev_tcp_init(&g_test_43e2.s_loop, &g_test_43e2.c_sock), 0);
}
...
TEST_FIXTURE_TEAREDOWN(tcp)
{
    ...
    ASSERT_EQ_D32(ev_loop_run(&g_test_43e2.s_loop, EV_LOOP_MODE_DEFAULT), 0);
    ASSERT_LOOP_EMPTY(&g_test_43e2.s_loop);
    ASSERT_EQ_D32(ev_loop_exit(&g_test_43e2.s_loop), 0);
}
...
static int _idle_client_setup_once_server(void)
{
    ...
    ASSERT_EQ_D32(ev_tcp_bind(&g_test_43e2.l_sock, (struct sockaddr*)&addr, sizeof(addr)), 0);

    size_t len = sizeof(addr);
    ASSERT_EQ_D32(ev_tcp_getsockname(&g_test_43e2.l_sock, (struct sockaddr*)&addr, &len), 0);

    int port = 0;
    ASSERT_EQ_D32(ev_ipv4_name(&addr, &port, NULL, 0), 0);
    ASSERT_GT_D32(port, 0);

    ASSERT_EQ_D32(ev_tcp_listen(&g_test_43e2.l_sock, 1024), 0);
    ASSERT_EQ_D32(ev_tcp_accept(&g_test_43e2.l_sock, &g_test_43e2.s_sock, _idle_client_on_accept), 0);
...
static void _idle_client_connect(int port)
{
    ...
    ASSERT_EQ_D32(ev_tcp_connect(&g_test_43e2.c_sock, (struct sockaddr*)&addr, sizeof(addr), _idle_client_on_connect), 0);
}
