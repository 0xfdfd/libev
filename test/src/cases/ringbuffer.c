/**
 * @file
 * This tests should be delete after IPC-ringbuffer finish.
 */
#include <stdlib.h>
#include <string.h>
#include "test.h"
...
TEST_FIXTURE_SETUP(ringbuffer)
{
}

TEST_FIXTURE_TEAREDOWN(ringbuffer)
{
}

TEST_F(ringbuffer, init_exit)
{
    const size_t cache_size = 1024;
    uint8_t* cache = malloc(cache_size);
    ASSERT_NE_PTR(cache, NULL);

    ring_buffer_t* rb = ring_buffer_init(cache, cache_size);
    ASSERT_NE_PTR(rb, (void*)NULL);

    ring_buffer_counter_t counter;
    ASSERT_EQ_SIZE(ring_buffer_count(rb, &counter), 0);
...
TEST_F(ringbuffer, consume_empty)
{
    ...
    ASSERT_EQ_PTR(ring_buffer_consume(rb), (void*)NULL);
 ...
 */
TEST_F(ringbuffer, consume_when_reserve_not_committed)
{
    const size_t cache_size = 1024;
    uint8_t* cache = malloc(cache_size);
    ASSERT_NE_PTR(cache, NULL);
    ...
    /* acquire write token */
    ring_buffer_token_t* write_token = ring_buffer_reserve(rb, 1, 0);
    ASSERT_NE_PTR(write_token, NULL);
    ASSERT_EQ_SIZE(ring_buffer_count(rb, &counter), 1);
    ASSERT_EQ_SIZE(counter.writing, 1);

    /* before `write_token` was committed, consume must failed */
    ASSERT_EQ_PTR(ring_buffer_consume(rb), NULL);
    ASSERT_EQ_SIZE(ring_buffer_count(rb, &counter), 1);
    ASSERT_EQ_SIZE(counter.writing, 1);

    /* commit write token */
    ASSERT_EQ_D32(ring_buffer_commit(rb, write_token, 0), 0);
    ASSERT_EQ_SIZE(ring_buffer_count(rb, &counter), 1);
    ASSERT_EQ_SIZE(counter.committed, 1);

    /* acquire read token must success */
    ring_buffer_token_t* read_token = ring_buffer_consume(rb);
    ASSERT_NE_PTR(read_token, NULL);
    ASSERT_EQ_SIZE(ring_buffer_count(rb, &counter), 1);
    ASSERT_EQ_SIZE(counter.reading, 1);

    ASSERT_EQ_D32(ring_buffer_commit(rb, read_token, 0), 0);
    ASSERT_EQ_SIZE(ring_buffer_count(rb, &counter), 0);
 ...
 */
TEST_F(ringbuffer, consume_when_reserve_not_committed2)
{
    /* make ring buffer */
    const size_t cache_size = 1024;
    uint8_t* cache = malloc(cache_size);
    ...
    ring_buffer_token_t* write_token_1 = ring_buffer_reserve(rb, 1, 0);
    ASSERT_NE_PTR(write_token_1, NULL);
    ASSERT_EQ_SIZE(ring_buffer_count(rb, &counter), 1);
    ASSERT_EQ_SIZE(counter.writing, 1);

    ring_buffer_token_t* write_token_2 = ring_buffer_reserve(rb, 1, 0);
    ASSERT_NE_PTR(write_token_2, NULL);
    ASSERT_EQ_SIZE(ring_buffer_count(rb, &counter), 2);
    ...
    ASSERT_EQ_D32(ring_buffer_commit(rb, write_token_2, 0), 0);
    ASSERT_EQ_SIZE(ring_buffer_count(rb, &counter), 2);
    ASSERT_EQ_SIZE(counter.writing, 1);
    ASSERT_EQ_SIZE(counter.committed, 1);
    ...
    ASSERT_EQ_D32(ring_buffer_commit(rb, write_token_1, 0), 0);
    ASSERT_EQ_SIZE(ring_buffer_count(rb, &counter), 2);
    ASSERT_EQ_SIZE(counter.committed, 2);

    /* because 1st write token was committed, `consume` is success */
    ring_buffer_token_t* consume_token = ring_buffer_consume(rb);
    ASSERT_NE_PTR(consume_token, NULL);
    ASSERT_EQ_SIZE(ring_buffer_count(rb, &counter), 2);
    ASSERT_EQ_SIZE(counter.committed, 1);
    ASSERT_EQ_SIZE(counter.reading, 1);

    ASSERT_EQ_D32(ring_buffer_commit(rb, consume_token, 0), 0);
    ASSERT_EQ_SIZE(ring_buffer_count(rb, &counter), 1);
    ASSERT_EQ_SIZE(counter.committed, 1);

    /* now 2st write token is available */
    ring_buffer_token_t* consume_token_2 = ring_buffer_consume(rb);
    ASSERT_NE_PTR(consume_token_2, NULL);
    ASSERT_EQ_D32(ring_buffer_commit(rb, consume_token_2, 0), 0);
    ASSERT_EQ_SIZE(ring_buffer_count(rb, &counter), 0);
 ...
 * 3. reserve (success)
 */
TEST_F(ringbuffer, consume_after_reserve_10K)
{
    const size_t cache_size = 1024;
    uint8_t* cache = malloc(cache_size);
    ASSERT_NE_PTR(cache, NULL);
    ...
    for (i = 0; i < 10000; i++)
    {
        uint64_t data = i;
        {
            ring_buffer_token_t* token = ring_buffer_reserve(rb, sizeof(uint64_t), 0);
            ...
            ASSERT_EQ_D32(ring_buffer_commit(rb, token, 0), 0);
        ...
        {
            ring_buffer_token_t* token = ring_buffer_consume(rb);
            ASSERT_NE_PTR(token, NULL);
            ASSERT_EQ_SIZE(token->size.size, sizeof(uint64_t));
 ...
 */
TEST_F(ringbuffer, max_element_no_overwrite)
{
    const size_t number_of_elements = 128;
    const size_t cache_size = ring_buffer_heap_cost() + number_of_elements * ring_buffer_node_cost(sizeof(uint64_t));
    uint8_t* cache = malloc(cache_size);
    ASSERT_NE_PTR(cache, NULL);
    ring_buffer_t* rb = ring_buffer_init(cache, cache_size);
    ASSERT_NE_PTR(rb, NULL);
    ...
    for (i = 0; i < number_of_elements; i++)
    {
        ...
        ASSERT_EQ_SIZE(ring_buffer_count(rb, &counter), i + 1);
        ASSERT_EQ_SIZE(counter.writing, 1);
    ...
    /* because ring buffer is full, reserve must failed */
    {
        ...
        ASSERT_EQ_SIZE(ring_buffer_count(rb, &counter), number_of_elements);
 ...
 */
TEST_F(ringbuffer, reserve_consume_8bit_full)
{
    ...
    ring_buffer_counter_t rb_counter;
    ASSERT_EQ_SIZE(ring_buffer_count(rb, &rb_counter), 0);
    ...
    for (;; token_counter++)
    {
        ...
        ASSERT_EQ_SIZE(ring_buffer_count(rb, &rb_counter), (size_t)(token_counter + 1));
    ...
    for (i = 0; i < token_counter; i++)
    {
        ...
        ASSERT_EQ_SIZE(ring_buffer_count(rb, &rb_counter), (size_t)(token_counter - i));
        ASSERT_EQ_SIZE(rb_counter.committed, (size_t)(token_counter - i - 1));
        ASSERT_EQ_SIZE(rb_counter.reading, 1);

        ASSERT_EQ_D32(ring_buffer_commit(rb, token, 0), 0);

        ASSERT_EQ_SIZE(ring_buffer_count(rb, &rb_counter), (size_t)(token_counter - i - 1));
        ASSERT_EQ_SIZE(rb_counter.committed, (size_t)(token_counter - i - 1));
    }
 ...
 */
TEST_F(ringbuffer, overwrite_generic)
{
    const size_t node_number = 5;
    const size_t cache_size = ring_buffer_heap_cost() + node_number * ring_buffer_node_cost(sizeof(uint64_t));
    uint8_t* cache = malloc(cache_size);
    ASSERT_NE_PTR(cache, NULL);
    ring_buffer_t* rb = ring_buffer_init(cache, cache_size);
    ASSERT_NE_PTR(rb, NULL);
    ...
    for (i = 0; i < node_number; i++)
    {
        ...
        ASSERT_EQ_SIZE(ring_buffer_count(rb, &rb_counter), (size_t)(i + 1));
        ASSERT_EQ_SIZE(rb_counter.writing, 1);
    ...
    /* check oldest data */
    {
        ...
        ASSERT_EQ_SIZE(ring_buffer_count(rb, &rb_counter), node_number);
        ...
        ASSERT_EQ_D32(ring_buffer_commit(rb, token, RINGBUFFER_FLAG_DISCARD), 0);
    ...
    /* overwrite oldest data */
    {
        ...
        ring_buffer_token_t* token = ring_buffer_reserve(rb, sizeof(uint64_t), RINGBUFFER_FLAG_OVERWRITE);
        ASSERT_NE_PTR(token, NULL);
    ...
    /* overwrite oldest data */
    {
        ...
        ring_buffer_token_t* token = ring_buffer_reserve(rb, sizeof(uint64_t) * 2, RINGBUFFER_FLAG_OVERWRITE);
        ASSERT_NE_PTR(token, NULL);

        ASSERT_EQ_SIZE(ring_buffer_count(rb, &rb_counter), node_number - 1);
 ...
 */
TEST_F(ringbuffer, overwrite_non_reserve)
{
    const size_t node_num = 5;
    const size_t cache_size = ring_buffer_heap_cost() + node_num * ring_buffer_node_cost(sizeof(uint64_t));
    uint8_t* cache = malloc(cache_size);
    ASSERT_NE_PTR(cache, NULL);
    ring_buffer_t* rb = ring_buffer_init(cache, cache_size);
    ASSERT_NE_PTR(rb, NULL);
    ...
    for (i = 0; i < node_num; i++)
    {
        ...
        ASSERT_EQ_SIZE(ring_buffer_count(rb, &rb_counter), i + 1);
    ...
    /* try to overwrite a node should failed */
    {
        ...
        ASSERT_EQ_SIZE(ring_buffer_count(rb, &rb_counter), node_num);
 ...
 */
TEST_F(ringbuffer, overwrite_only_one)
{
    const size_t cache_size = ring_buffer_heap_cost() + ring_buffer_node_cost(100);
    uint8_t* cache = malloc(cache_size);
    ASSERT_NE_PTR(cache, NULL);
    ring_buffer_t* rb = ring_buffer_init(cache, cache_size);
    ASSERT_NE_PTR(rb, NULL);
    ...
    /* reserve 100 byte */
    {
        ring_buffer_token_t* token = ring_buffer_reserve(rb, 100, 0);
        ASSERT_NE_PTR(token, NULL);

        ASSERT_EQ_SIZE(ring_buffer_count(rb, &rb_counter), 1);
        ASSERT_EQ_SIZE(rb_counter.writing, 1);
    ...
    {
        ASSERT_EQ_PTR(ring_buffer_reserve(rb, 1, 0), NULL);
    ...
    /* overwrite */
    {
        ring_buffer_token_t* token = ring_buffer_reserve(rb, 1, RINGBUFFER_FLAG_OVERWRITE);
        ASSERT_NE_PTR(token, NULL);
 ...
 */
TEST_F(ringbuffer, write_discard_only_one)
{
    ...
    ring_buffer_counter_t rbc;
    ASSERT_EQ_SIZE(ring_buffer_count(rb, &rbc), 0);
    ...
    /* acquire a write token and discard it */
    {
        ...
        ASSERT_EQ_SIZE(ring_buffer_count(rb, &rbc), 1);
 ...
 */
TEST_F(ringbuffer, consume_discard_only_one)
{
    const size_t cache_size = 1 * 1024;
    uint8_t* cache = malloc(cache_size);
    ASSERT_NE_PTR(cache, NULL);
    ...
    {
        ring_buffer_token_t* token = ring_buffer_reserve(rb, sizeof(tmpval), 0);
        ASSERT_NE_PTR(token, NULL);
        ASSERT_EQ_SIZE(token->size.size, sizeof(tmpval));
 ...
 */
TEST_F(ringbuffer, discard_older_consume)
{
    const size_t cache_size = 1 * 1024;
    uint8_t* cache = malloc(cache_size);
    ...
    for (;;)
    {
        ...
        ASSERT_EQ_SIZE(ring_buffer_count(rb, &rbc), number_of_ele + 1);
    ...
    ring_buffer_token_t* token_1 = ring_buffer_consume(rb);
    ASSERT_NE_PTR(token_1, NULL);

    ASSERT_EQ_SIZE(ring_buffer_count(rb, &rbc), number_of_ele);
    ASSERT_EQ_SIZE(rbc.committed, number_of_ele - 1);
    ASSERT_EQ_SIZE(rbc.reading, 1);

    ring_buffer_token_t* token_2 = ring_buffer_consume(rb);
    ...
    ASSERT_LT_D32(ring_buffer_commit(rb, token_1, RINGBUFFER_FLAG_DISCARD), 0);
    ASSERT_EQ_SIZE(ring_buffer_count(rb, &rbc), number_of_ele);
    ASSERT_EQ_SIZE(rbc.committed, number_of_ele - 2);
    ASSERT_EQ_SIZE(rbc.reading, 2);

    /* discard 2st token should success */
    ASSERT_EQ_D32(ring_buffer_commit(rb, token_2, RINGBUFFER_FLAG_DISCARD), 0);
    ASSERT_EQ_SIZE(ring_buffer_count(rb, &rbc), number_of_ele);
    ASSERT_EQ_SIZE(rbc.committed, number_of_ele - 1);
    ASSERT_EQ_SIZE(rbc.reading, 1);

    /* now discard 1st token should success */
    ASSERT_EQ_D32(ring_buffer_commit(rb, token_1, RINGBUFFER_FLAG_DISCARD), 0);
    ASSERT_EQ_SIZE(ring_buffer_count(rb, &rbc), number_of_ele);
    ASSERT_EQ_SIZE(rbc.committed, number_of_ele);
 ...
 */
TEST_F(ringbuffer, force_discard_older_consume)
{
    const size_t number_of_elements = 2;
    const size_t cache_size = ring_buffer_heap_cost() + number_of_elements * ring_buffer_node_cost(sizeof(uint32_t));
    uint8_t* cache = malloc(cache_size);
    ASSERT_NE_PTR(cache, NULL);
    ring_buffer_t* rb = ring_buffer_init(cache, cache_size);
    ASSERT_NE_PTR(rb, NULL);
    ...
    for (i = 0; i < number_of_elements; i++)
    {
        ring_buffer_token_t* token = ring_buffer_reserve(rb, sizeof(i), 0);
        ASSERT_NE_PTR(token, NULL);
        memcpy(token->data, &i, sizeof(i));

        ASSERT_EQ_SIZE(ring_buffer_count(rb, &rbc), i + (size_t)1);
    ...
    ASSERT_EQ_SIZE(ring_buffer_count(rb, &rbc), number_of_elements);
    ASSERT_EQ_SIZE(rbc.reading, 1);
    ...
    /* discard 1st token */
    {
        ...
        ASSERT_EQ_D32(ring_buffer_commit(rb, token_1, RINGBUFFER_FLAG_DISCARD | RINGBUFFER_FLAG_ABANDON), 0);
        ASSERT_EQ_SIZE(ring_buffer_count(rb, &rbc), number_of_elements - 1);
        ASSERT_EQ_SIZE(rbc.reading, 1);
        ASSERT_EQ_SIZE(rbc.committed, number_of_elements - 2);
    }
    ...
    /* commit 2st token */
    {
        ASSERT_EQ_D32(ring_buffer_commit(rb, token_2, 0), 0);
        ASSERT_EQ_SIZE(ring_buffer_count(rb, &rbc), number_of_elements - 2);
 ...
 */
TEST_F(ringbuffer, buffer_too_small)
{
    ...
    ASSERT_EQ_PTR(ring_buffer_init(cache, cache_size), NULL);
 ...
 */
TEST_F(ringbuffer, reserve_empty_but_too_large)
{
    const size_t cache_size = 1 * 1024;
    uint8_t* cache = malloc(cache_size);
    ASSERT_NE_PTR(cache, NULL);
    ...
    ring_buffer_token_t* token = ring_buffer_reserve(rb, cache_size + 1, 0);
    ASSERT_EQ_PTR(token, NULL);
 ...
 */
TEST_F(ringbuffer, discard_middle)
{
    const size_t cache_size = ring_buffer_heap_cost() + 3 * ring_buffer_node_cost(sizeof(uint64_t));
    uint8_t* cache = malloc(cache_size);
    ASSERT_NE_PTR(cache, NULL);
    ring_buffer_t* rb = ring_buffer_init(cache, cache_size);
    ASSERT_NE_PTR(rb, (void*)NULL);
    ...
    /* acquire 3 tokens */
    ring_buffer_token_t* token_1 = ring_buffer_reserve(rb, sizeof(uint64_t), 0);
    ASSERT_EQ_SIZE(ring_buffer_count(rb, &rbc), 1);
    ASSERT_EQ_SIZE(rbc.writing, 1);
    ASSERT_NE_PTR(token_1, (void*)NULL);

    ring_buffer_token_t* token_2 = ring_buffer_reserve(rb, sizeof(uint64_t), 0);
    ASSERT_EQ_SIZE(ring_buffer_count(rb, &rbc), 2);
    ASSERT_EQ_SIZE(rbc.writing, 2);
    ASSERT_NE_PTR(token_2, (void*)NULL);

    ring_buffer_token_t* token_3 = ring_buffer_reserve(rb, sizeof(uint64_t), 0);
    ASSERT_EQ_SIZE(ring_buffer_count(rb, &rbc), 3);
    ASSERT_EQ_SIZE(rbc.writing, 3);
    ASSERT_NE_PTR(token_3, (void*)NULL);

    /* the 4st token should not be reserved */
    ASSERT_EQ_PTR(ring_buffer_reserve(rb, sizeof(uint64_t), 0), NULL);
    ASSERT_EQ_SIZE(ring_buffer_count(rb, &rbc), 3);
    ASSERT_EQ_SIZE(rbc.writing, 3);
    ...
    /* write to 1st token */
    {
        ...
        ASSERT_EQ_D32(ring_buffer_commit(rb, token_1, 0), 0);
    ...
    /* write to 3st token */
    {
        ...
        ASSERT_EQ_D32(ring_buffer_commit(rb, token_3, 0), 0);
    ...
    /* check 1st data */
    ring_buffer_token_t* r_token_1 = NULL;
    {
        r_token_1 = ring_buffer_consume(rb);
    ...
    /* check 2st data */
    ring_buffer_token_t* r_token_2 = NULL;
    {
        r_token_2 = ring_buffer_consume(rb);
    ...
    /* commit consume */
    {
        ASSERT_EQ_D32(ring_buffer_commit(rb, r_token_2, 0), 0);
        ASSERT_EQ_SIZE(ring_buffer_count(rb, &rbc), 1);
        ASSERT_EQ_SIZE(rbc.reading, 1);

        ASSERT_EQ_D32(ring_buffer_commit(rb, r_token_1, 0), 0);
        ASSERT_EQ_SIZE(ring_buffer_count(rb, &rbc), 0);
    }
 ...
 */
TEST_F(ringbuffer, discard_newest)
{
    ...
    for (i = 0; i < number_of_elements - 1; i++)
    {
        ...
        ASSERT_EQ_SIZE(ring_buffer_count(rb, &rbc), (size_t)(i + 1));
        ASSERT_EQ_SIZE(rbc.writing, 1);
...
TEST_F(ringbuffer, counter_empty)
{
    const size_t number_of_elements = 3;
    const size_t cache_size = ring_buffer_heap_cost() + number_of_elements * ring_buffer_node_cost(sizeof(uint64_t));
    uint8_t* cache = malloc(cache_size);
...
TEST_F(ringbuffer, counter_full)
{
    const size_t number_of_elements = 3;
    const size_t cache_size = ring_buffer_heap_cost() + number_of_elements * ring_buffer_node_cost(sizeof(uint64_t));
    uint8_t* cache = malloc(cache_size);
...
TEST_F(ringbuffer, foreach)
{
    ...
    /* work through */
    {
        ring_buffer_token_t* token = ring_buffer_begin(rb);
        ASSERT_NE_PTR(token, NULL);

        for (i = 0; token != NULL; token = ring_buffer_next(rb, token), i++)
        {
