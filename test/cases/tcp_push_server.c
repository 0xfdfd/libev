...
TEST_F(tcp, push_server)
{
    ...
    ASSERT_EQ_INT(ev_tcp_bind(&g_test_6d69->s_server, (struct sockaddr*)&addr, sizeof(addr)), 0);
    ASSERT_EQ_INT(ev_tcp_listen(&g_test_6d69->s_server, 1), 0);
    ASSERT_EQ_INT(ev_tcp_accept(&g_test_6d69->s_server, &g_test_6d69->s_conn, _on_accept_6d69), 0);

    /* Get listen port */
    size_t len = sizeof(addr);
    ASSERT_EQ_INT(ev_tcp_getsockname(&g_test_6d69->s_server, (struct sockaddr*)&addr, &len), 0);
    ASSERT_EQ_INT(ev_ipv4_name(&addr, &g_test_6d69->s_listen_port, NULL, 0), 0);

    /* Connect to listen socket */
    ASSERT_EQ_INT(ev_ipv4_addr("127.0.0.1", g_test_6d69->s_listen_port, &addr), 0);
    ASSERT_EQ_INT(ev_tcp_connect(&g_test_6d69->s_client, (struct sockaddr*)&addr,
