...
TEST_FIXTURE_TEARDOWN(misc)
{
    ...
    ASSERT_EQ_INT(ev_loop_run(&g_test_09d8.s_loop, EV_LOOP_MODE_DEFAULT, EV_INFINITE_TIMEOUT), 0);
    ASSERT_EQ_EVLOOP(&g_test_09d8.s_loop, &empty_loop);
...
TEST_F(misc, ipv4_addr)
{
    ...
    ASSERT_EQ_INT(ev_tcp_bind(&g_test_09d8.s_socket, (struct sockaddr*)&addr, sizeof(addr)), 0);

    memset(&addr, 0, sizeof(addr));
    ASSERT_EQ_INT(ev_tcp_getsockname(&g_test_09d8.s_socket, (struct sockaddr*)&addr, &len), 0);

    ASSERT_EQ_INT(ev_ipv4_name(&addr, NULL, g_test_09d8.s_buffer, sizeof(g_test_09d8.s_buffer)), 0);
