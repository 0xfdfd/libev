...
{
    ev_tcp_exit(g_test_7145.s_sock, NULL, NULL);
    ev_tcp_exit(g_test_7145.c_sock, NULL, NULL);

    ASSERT_EQ_INT(ev_loop_run(g_test_7145.s_loop, EV_LOOP_MODE_DEFAULT,
                              EV_INFINITE_TIMEOUT),
                  0);
    ASSERT_EQ_INT(ev_loop_exit(g_test_7145.s_loop), 0);
}

static void _connect_non_exist_on_ret(ev_tcp_t *sock, int stat, void *arg)
{
    (void)sock;
    (void)arg;
...
TEST_T(tcp, connect_non_exist, 1000)
{
    ...
    ASSERT_LE_INT(ev_tcp_connect(g_test_7145.c_sock, (struct sockaddr *)&addr,
                                 sizeof(addr), _connect_non_exist_on_ret, NULL),
                  0);
    ASSERT_EQ_INT(ev_loop_run(g_test_7145.s_loop, EV_LOOP_MODE_DEFAULT,
                              EV_INFINITE_TIMEOUT),
                  0);
