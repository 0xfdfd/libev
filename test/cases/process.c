...
static void _close_stdin_pipe(void)
{
    if (g_test_process->flag_stdin_init)
    {
        ...
        ev_pipe_exit(g_test_process->stdin_pipe, NULL, NULL);
    }
}
...
TEST_FIXTURE_SETUP(process)
{
    ...
    ASSERT_EQ_INT(
        ev_pipe_init(g_test_process->loop, &g_test_process->stdin_pipe, 0), 0);
    ASSERT_EQ_INT(
        ev_pipe_init(g_test_process->loop, &g_test_process->stdout_pipe, 0), 0);
    ASSERT_EQ_INT(
        ev_pipe_init(g_test_process->loop, &g_test_process->stderr_pipe, 0), 0);
...
{
    _close_stdin_pipe();
    ev_pipe_exit(g_test_process->stdout_pipe, NULL, NULL);
    ev_pipe_exit(g_test_process->stderr_pipe, NULL, NULL);

    ASSERT_EQ_INT(ev_loop_run(g_test_process->loop, EV_LOOP_MODE_DEFAULT,
                              EV_INFINITE_TIMEOUT),
                  0);
...
TEST_F(process, spawn_stdout_ignore)
{
    ...
    ret =
        ev_process_spawn(g_test_process->loop, &g_test_process->process, &opt);
    ASSERT_EQ_INT(ret, 0);
...
static void _test_process_redirect_pipe_on_write_done(ev_pipe_t *pipe,
...
TEST_F(process, redirect_pipe)
{
    ...
    ev_buf_t write_buf =
        ev_buf_make((char *)g_test_process_data, strlen(g_test_process_data));
    ret = ev_pipe_write(g_test_process->stdin_pipe, &write_buf, 1,
                        _test_process_redirect_pipe_on_write_done, NULL);
    ASSERT_EQ_INT(ret, 0);
...
static void _process_cwd_on_read(ev_pipe_read_req_t *req, ssize_t size)
{
    process_cwd_path_t *cwd_path =
        EV_CONTAINER_OF(req, process_cwd_path_t, req);
